<script lang="ts">
	...
	let finalAmount = $derived(donationAmount === 0 ? Number(customAmount) || 0 : donationAmount);

	const pixKey =
		'00020101021226840014br.gov.bcb.pix2562pix.ordinarium.com/apoio520400005303986540510.005802BR5910Ordinarium6009Sao Paulo62070503***63041A2D';
	...
	<div class="flex flex-col lg:flex-row gap-8 items-stretch w-full max-w-5xl mx-auto mt-2">
			...
			<div class="space-y-4">
				<div class="flex items-center gap-2">
					<span class="text-[10px] uppercase font-bold tracking-widest text-brand-gold font-sans"
						>Apoio Financeiro</span
					>
				</div>
				<h2 class="text-2xl font-bold text-text-main tracking-tight font-sans">
					Manter o Projeto Vivo e Independente
				</h2>
				<p class="text-xs text-text-muted leading-relaxed font-sans">
					O Ordinarium nasceu como uma iniciativa voluntária e gratuita para ajudar os fiéis a
					encontrarem a Santa Missa e Confissões de forma simples e digna.
				</p>
				<p class="text-xs text-text-muted leading-relaxed font-sans">
					Para mantermos o projeto ativo e livre de anúncios comerciais inconvenientes, dependemos
					do apoio espontâneo da comunidade para cobrir custos de:
				</p>
			...
			<div class="p-3 bg-bg-dark/45 border border-border-dark rounded-xl flex items-center gap-3">
				...
				<div class="text-left font-sans">
					...
					<p class="text-[10px] text-text-muted">
						100% dos recursos são aplicados na infraestrutura do portal.
					</p>
				</div>
			</div>
		</div>

		<!-- Right: Interactive Support Donation Form -->
		<div
		...
		>
				...
				<div class="space-y-4 text-left">
					...
					{#if donationAmount === 0}
						<div class="space-y-1.5 animate-fade-in">
							<label for="customDonation" class="text-xs font-semibold text-text-muted block"
								>Digite o Valor (R$)</label
							>
							<input
								id="customDonation"
								type="number"
					...
					<form
						...
						class="pt-4 border-t border-border-dark/60 mt-4 space-y-3"
					>
						{#if paymentMethod === 'card'}
							<div class="space-y-2.5 animate-fade-in">
								<div class="space-y-1">
									<label for="cardNum" class="text-[10px] font-semibold text-text-muted block"
								...
								<div class="flex gap-2">
									<div class="flex-1 space-y-1">
										<label for="cardVal" class="text-[10px] font-semibold text-text-muted block"
											>Validade</label
										>
										<input
											id="cardVal"
									...
									<div class="flex-1 space-y-1">
										<label for="cardCvc" class="text-[10px] font-semibold text-text-muted block"
											>CVV</label
										>
										<input
											id="cardCvc"
			...
			{:else if paymentStatus === 'loading'}
				...
				<div
					class="flex flex-col items-center justify-center py-20 text-center space-y-4 font-sans animate-fade-in my-auto"
				>
			...
			{:else if paymentStatus === 'success'}
				...
				<div class="space-y-4 text-left animate-fade-in font-sans">
					<div class="flex items-center gap-2">
						<div
							class="h-6 w-6 flex items-center justify-center rounded bg-brand-gold-dim border border-brand-gold/25"
						>
							<Check class="h-3.5 w-3.5 text-brand-gold" />
						</div>
						<span class="text-[10px] uppercase font-bold tracking-widest text-brand-gold"
							>Apoio Confirmado</span
						>
					</div>
					...
					{#if paymentMethod === 'pix'}
						...
						<div class="space-y-3">
							<h3 class="text-lg font-bold text-text-main tracking-tight leading-snug">
								Escaneie o QR Code Pix
							</h3>
							<p class="text-xs text-text-muted">
								Aponte a câmera do seu app do banco para o QR Code abaixo para doar **R$ {finalAmount}**.
							</p>

							<!-- QR code box -->
							<div
								class="mx-auto h-36 w-36 bg-white p-2.5 rounded-xl border border-border-dark flex items-center justify-center shadow-inner mt-2"
							>
								<div
									class="w-full h-full border-2 border-dashed border-zinc-300 flex items-center justify-center text-zinc-400 font-mono text-[9px] text-center select-none"
								>
							...
							<div class="space-y-1.5 pt-2">
								<span class="text-[10px] font-semibold text-text-muted block"
									>Código Pix Copia e Cola</span
								>
								<div class="flex gap-2">
									<input
										type="text"
					...
					{:else}
						...
						<div class="space-y-2 py-6 text-center">
							<div
								class="h-12 w-12 rounded-full bg-brand-gold-dim border border-brand-gold/20 flex items-center justify-center mx-auto mb-3"
							>
								<Check class="h-6 w-6 text-brand-gold" />
							</div>
							<h3 class="text-lg font-bold text-text-main tracking-tight">
								Obrigado pela Contribuição!
							</h3>
							<p class="text-xs text-text-muted leading-relaxed">
								Sua doação de **R$ {finalAmount}** no cartão foi processada com sucesso.
							</p>
						</div>
					{/if}
