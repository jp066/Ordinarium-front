<script lang="ts">
	...
	let filteredChurches = $derived(
		churches.filter((church: Church) => {
			...
			return church.schedules.some(
				(s: { type: string; day: string; time: string }) => s.day === targetDay
			);
		})
	);
...
<div
	class="flex-1 w-full min-h-0 overflow-y-auto flex flex-col justify-start items-center py-6 px-6 sm:px-12 gap-6 max-w-7xl mx-auto"
>
	<!-- Devotional Panel (Liturgical Hours) -->
	<div
		class="w-full max-w-2xl bg-bg-card border border-border-dark shadow-xl rounded-2xl overflow-hidden flex flex-col sm:flex-row h-auto sm:h-40 shrink-0 transition-all duration-200 hover:border-brand-gold/30"
	>
		<!-- Left/Top Side: Image with Liturgical Filters -->
		<div
			class="relative w-full sm:w-54 h-50 sm:h-full bg-bg-dark overflow-hidden select-none border-b sm:border-b-0 sm:border-r border-border-dark/50 shrink-0"
		>
			<img
			...
			/>
			<div
				class="absolute inset-0 bg-gradient-to-t sm:bg-gradient-to-r from-transparent via-bg-card/25 to-bg-card/90"
			></div>
		</div>

		<!-- Right/Bottom Side: Text Content -->
		<div class="flex-1 p-5 flex flex-col justify-center gap-1.5 select-none text-left">
			<span class="text-[12px] font-bold tracking-widest text-brand-gold font-sans block"
				>Reze agora as</span
			>
	...
	<div
		class="flex flex-wrap justify-center gap-1 border-b border-border-dark/60 pb-px w-fit mx-auto mt-1 shrink-0 max-w-full"
	>
		...
		<div bind:this={carouselElement} class="carousel-container mt-2 w-full">
			{#each filteredChurches as church}
				...
				>
					...
					<div
						class="absolute inset-0 z-0 pointer-events-none select-none overflow-hidden transition-all duration-300 {isActive
							? 'opacity-[0.8] dark:opacity-[0.2]'
							: 'opacity-[0.2] dark:opacity-[0.1]'}"
					>
						<img
							src={church.bgImage}
							alt=""
						...
						/>
						<div
							class="absolute inset-0 bg-gradient-to-t from-bg-card/95 via-bg-card/75 to-bg-card/20 dark:from-bg-card/98 dark:via-bg-card/85 dark:to-bg-card/40"
						></div>
					</div>
					...
					<div class="relative z-10 flex flex-col gap-3 w-full">
						...
						<div class="space-y-1 select-none">
							<span
								class="text-[9px] uppercase font-bold tracking-widest text-brand-gold block font-sans"
								>Paróquia</span
							>
							<h3
								class="text-md md:text-lg font-bold text-text-main tracking-tight leading-snug line-clamp-1 font-sans"
							>
								{church.name}
							</h3>
						</div>
						...
						<div class="space-y-1.5 text-text-muted text-xs select-none">
							{#if isActive}
								...
								>
									<MapPin
										size={13}
										class="text-text-muted group-hover:text-brand-gold shrink-0 mt-0.5"
									/>
									<span class="leading-normal line-clamp-2 group-hover:underline"
										>{church.address}</span
									>
								</div>
							...
							{:else}
								<!-- Summary (Inactive) -->
								<p class="text-xs text-text-muted line-clamp-2 leading-relaxed font-sans">
									{church.address}
								</p>
							{/if}
						</div>
						...
						{#if isActive}
							<div class="pt-3 border-t border-border-dark/30 w-full select-none">
								<span
									class="text-[9px] font-bold uppercase tracking-widest text-brand-gold block mb-2 font-sans"
									>Próximos Horários</span
								>
								<div class="space-y-1.5 max-h-32 overflow-y-auto pr-1">
									{#each getSchedulesForChurch(church.id).slice(0, 3) as item}
										{@const Icon = getIcon(item.type)}
										<div
											class="flex items-center justify-between text-xs py-1 px-2.5 rounded bg-bg-dark/40 border border-border-dark/50 font-sans"
										>
											<span class="font-bold text-text-main font-mono"
												>{item.timeStr}
												<span class="text-text-muted font-sans font-normal">({item.dateStr})</span
												></span
											>
									...
									{#if getSchedulesForChurch(church.id).length === 0}
										<span class="text-xs text-text-dim block font-sans"
					...
					<!-- Footer details -->
					<div
						class="border-t border-border-dark/30 pt-3 mt-4 flex justify-between items-center w-full relative z-10 select-none"
					>
						<span class="text-[9px] text-text-dim font-bold uppercase font-sans">Grade Semanal</span
						>
						<span class="tag tag-gold text-[9px] py-0.5 px-2 font-sans"
							>{church.schedules.length} horários</span
						>
					</div>
				</div>
			{/each}
			...
			{#if filteredChurches.length === 0}
				<div class="flex flex-col items-center justify-center py-12 text-center w-full">
					<p class="text-xs text-text-muted font-medium font-sans">
						Nenhuma paróquia com horários cadastrados nesta categoria.
					</p>
				</div>
			{/if}
		</div>
	...
	<div class="flex flex-col md:hidden w-full gap-3 px-1 mt-2 select-none">
		{#each filteredChurches as church (church.id)}
			...
			<div
				...
				{isExpanded ? 'border-brand-gold gold-glow-active' : 'border-border-dark/60 shadow-md'}"
			>
				...
				>
					<div class="flex-1 min-w-0 pr-3">
						<span
							class="text-[9px] uppercase tracking-widest text-brand-gold font-bold font-sans block mb-0.5"
							>Paróquia</span
						>
						<h3
							class="text-sm font-bold text-text-main leading-tight font-sans tracking-tight truncate"
						>
							{church.name}
						</h3>
					...
					<div class="flex items-center gap-3 shrink-0">
						<span
							class="text-[10px] font-bold text-brand-gold bg-brand-gold-dim/10 px-2 py-0.5 rounded border border-brand-gold/15"
						>
							{churchSchedules.length}
							{churchSchedules.length === 1 ? 'horário' : 'horários'}
						</span>
						{#if isExpanded}
				...
				{#if isExpanded}
					<div
						class="px-4 pb-4 border-t border-border-dark/30 pt-3 flex flex-col gap-4 animate-fade-in font-sans"
					>
						<!-- Actions row -->
						...
						<div class="space-y-1.5 max-h-48 overflow-y-auto pr-1">
							{#each churchSchedules as sched}
								...
								<div
									class="flex items-center justify-between text-xs py-1.5 border-b border-border-dark/20"
								>
									<div class="flex items-center gap-2">
										<span class="font-bold text-text-main font-mono"
											>{sched.timeStr}
											<span class="text-text-muted font-normal font-sans">({sched.dateStr})</span
											></span
										>
									</div>
									<div
										class="flex items-center gap-1 text-[10px] text-brand-gold font-medium bg-brand-gold-dim/10 px-2 py-0.5 rounded border border-brand-gold/15"
									>
							...
							{:else}
								<span class="text-xs text-text-muted italic block py-4 text-center"
									>Nenhum horário correspondente.</span
								>
							{/each}
						</div>
						...
						<div
							class="pt-3 border-t border-border-dark/30 flex items-center justify-between text-[10px] text-text-muted"
						>
							<span>Grade Semanal</span>
							<span class="font-bold text-brand-gold"
								>{church.schedules.length} horários cadastrados</span
							>
						</div>
					</div>
				{/if}
		...
		{:else}
			<div
				class="text-center py-10 font-sans border border-dashed border-border-dark/60 rounded-2xl p-6 bg-bg-card/25 select-none"
			>
				<span class="text-sm text-text-muted block"
					>Nenhuma paróquia encontrada para este filtro.</span
				>
			</div>
		{/each}
	</div>
