...
<SEO 
	...
	keywords={['api católica', 'desenvolvedores', 'ordinarium api', 'dados abertos católicos', 'desenvolvedor católico', 'rest api', 'json api']}
/>

<div class="h-screen w-screen bg-bg-dark text-text-main font-sans flex flex-col overflow-hidden">

	<!-- Top Bar (fixed header for the doc page) -->
	<div class="h-14 shrink-0 border-b border-border-dark bg-bg-sidebar/90 backdrop-blur-md flex items-center justify-between px-6 z-10">
		<div class="flex items-center gap-3">
			...
			<div class="flex items-center gap-2">
				...
				<span class="text-[10px] px-1.5 py-0.5 rounded bg-[#10b981]/15 text-[#10b981] border border-[#10b981]/25 font-bold uppercase tracking-wider">v1</span>
			</div>
		</div>
	...
	<div class="flex flex-1 overflow-hidden">
		...
		<aside class="hidden md:flex flex-col w-56 shrink-0 border-r border-border-dark bg-bg-sidebar/60 overflow-y-auto py-6 px-3 gap-1 select-none">
			<!-- Sidebar header -->
			<p class="text-[10px] font-bold uppercase tracking-widest text-text-muted/50 px-3 mb-2">Navegação</p>
			...
			{#each navSections as section}
				...
				>
					<section.icon size={13} class={activeSection === section.id ? 'text-brand-gold' : 'text-text-muted/50'} />
					<span class="leading-tight">{section.label}</span>
					{#if section.tag}
						<span class="ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#10b981]/15 text-[#10b981] border border-[#10b981]/20 shrink-0">{section.tag}</span>
					{/if}
				</button>
			{/each}
		...
		<main class="flex-1 overflow-y-auto px-6 sm:px-10 py-10 flex flex-col gap-12">
			...
			<section id="intro" class="flex flex-col gap-5">
				<div class="flex items-center gap-3">
					<div class="h-9 w-9 rounded-xl bg-brand-gold-dim flex items-center justify-center border border-brand-gold/20 shrink-0">
				...
				<p class="text-sm text-text-muted leading-relaxed max-w-2xl">
					A API do Ordinarium é uma interface REST pública e gratuita que expõe os dados litúrgicos do portal: paróquias registradas, horários de Missas, Confissões e Adorações. Todos os endpoints retornam <code class="font-mono text-[11px] bg-bg-card border border-border-dark rounded px-1.5 py-0.5 text-text-main">JSON</code>.
				</p>
				<div class="bg-bg-card border border-border-dark rounded-xl p-4 flex flex-col gap-2 text-xs text-text-muted max-w-2xl">
			...
			<section id="auth" class="flex flex-col gap-5">
				<div class="flex items-center gap-3">
					<div class="h-9 w-9 rounded-xl bg-bg-card flex items-center justify-center border border-border-dark shrink-0">
				...
				<p class="text-sm text-text-muted leading-relaxed max-w-2xl">
					Todos os endpoints são <strong class="text-text-main font-semibold">públicos</strong> e não requerem nenhuma chave de API ou cabeçalho de autorização. Basta realizar requisições HTTP <code class="font-mono text-[11px] bg-bg-card border border-border-dark rounded px-1.5 py-0.5 text-[#10b981]">GET</code> diretamente.
				</p>
				<div class="bg-bg-card border border-[#10b981]/25 rounded-xl p-4 flex items-start gap-3 text-xs max-w-2xl">
					<span class="text-[#10b981] text-base shrink-0 mt-0.5">✓</span>
					<p class="text-text-muted leading-relaxed">Nenhum cabeçalho especial necessário. CORS habilitado para todos os domínios.</p>
				</div>
			</section>
			...
			<section id="churches" class="flex flex-col gap-5">
				<div class="flex items-center gap-3">
					<span class="text-xs font-bold uppercase px-2.5 py-1.5 rounded-lg bg-[#10b981]/15 text-[#10b981] border border-[#10b981]/25 shrink-0">GET</span>
					<h2 class="text-xl font-bold text-text-main font-mono tracking-tight">/api/churches</h2>
				</div>

				<p class="text-sm text-text-muted leading-relaxed max-w-2xl">
					Retorna todas as paróquias registradas no banco de dados do Ordinarium, incluindo endereço, dados de contato, coordenadas geográficas e a grade geral de horários.
				</p>

				<!-- Response card -->
				<div class="bg-bg-card border border-border-dark shadow-lg rounded-2xl overflow-hidden max-w-3xl">
					<div class="flex items-center justify-between px-5 py-3 border-b border-border-dark/60">
						<div class="flex items-center gap-2">
							<span class="text-[10px] font-bold uppercase tracking-wider text-text-muted/60">Resposta de Exemplo</span>
							<span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-bg-dark border border-border-dark text-text-muted">200 OK</span>
						</div>
						<div class="flex items-center gap-2">
							<a href="/api/churches" target="_blank" class="text-[11px] font-semibold text-brand-gold hover:underline flex items-center gap-1.5 transition-colors cursor-pointer">
								<span>Testar</span>
					...
					<div class="relative p-5 text-[11px] font-mono text-left text-[#f3f4f6] max-h-72 overflow-y-auto leading-relaxed bg-bg-dark/85">
						<button
			...
			<section id="schedules" class="flex flex-col gap-5 pb-16">
				...
				<p class="text-sm text-text-muted leading-relaxed max-w-2xl">
					Retorna uma lista plana e ordenada com todos os eventos litúrgicos (Missas, Confissões, Adorações) agendados pelas igrejas, detalhando dia da semana, strings legíveis, ID da igreja e notas adicionais de curadoria.
				</p>
				...
				<div class="bg-bg-card border border-border-dark shadow-lg rounded-2xl overflow-hidden max-w-3xl">
					<div class="flex items-center justify-between px-5 py-3 border-b border-border-dark/60">
						...
						<div class="flex items-center gap-2">
							<a href="/api/schedules" target="_blank" class="text-[11px] font-semibold text-brand-gold hover:underline flex items-center gap-1.5 transition-colors cursor-pointer">
								<span>Testar</span>
								<ExternalLink size={11} />
							</a>
							<button
