<script lang="ts">
	...
	const menuItems = [
		...
		{ name: 'Orações', icon: Sparkles, href: '/oracoes' },
		{ name: 'Apoiar', icon: Heart, href: '/apoiar' },
		{ name: 'Sugerir', icon: MessageSquare, href: '/sugerir' }
	];
	...
	$effect(() => {
		...
		if (path === '/horarios') {
			...
			navState.activeRoute = 'Orações';
		} else if (path === '/apoiar') {
			navState.activeRoute = 'Apoiar';
...
{:else}
	<div
		class="flex flex-col h-screen w-screen overflow-hidden bg-bg-dark text-text-main transition-colors duration-200 relative"
	>
		<!-- Animated Background Texture (Giphy) -->
		<div
			class="absolute inset-0 z-0 pointer-events-none select-none opacity-[0.08] overflow-hidden"
		>
		...
		<header
			class="sticky top-0 z-50 flex h-16 w-full items-center justify-between border-b border-border-dark bg-bg-sidebar/95 backdrop-blur-md px-6 sm:px-12 transition-all duration-200 shrink-0"
		>
			...
			>
				...
				<h1 class="text-2xl font-bold tracking-wide text-brand-gold font-gothic select-none">
					Ordinarium
				</h1>
			</a>
		...
		{#if showMobileMenu}
			<div
				class="fixed top-16 left-0 w-full bg-bg-sidebar/95 backdrop-blur-lg border-b border-border-dark flex flex-col p-4 gap-2.5 z-[1002] animate-fade-in md:hidden select-none shadow-2xl"
			>
				<a
					href="/"
					onclick={() => (showMobileMenu = false)}
