...
>
	{#if !data.success}
		...
		>
			...
			<p class="text-xs text-text-muted mt-2 leading-relaxed">
				{data.error ||
					'Ocorreu um erro ao buscar a Liturgia Diária deste dia. Verifique sua conexão ou tente novamente mais tarde.'}
			</p>
	...
	{:else}
		...
		<div
			class="w-full flex flex-col md:flex-row md:items-center justify-between gap-4 py-3 select-none border-b border-border-dark/40 shrink-0"
		>
			...
			{#if activeCelebration}
				<div
					class="flex items-center gap-1.5 rounded-xl p-1 border border-border-dark bg-bg-dark self-start md:self-center select-none shadow-sm"
				>
					<button
		...
		{#if liturgia.celebracoes.length > 1}
			<div
				class="w-full flex flex-col gap-2 bg-bg-card/30 border border-border-dark p-3 rounded-2xl select-none shrink-0"
			>
				<span class="text-[9px] uppercase font-bold tracking-wider text-text-muted/50 px-1"
					>Celebrações do Dia</span
				>
				<div class="flex flex-wrap gap-2">
					{#each liturgia.celebracoes as cel, idx}
						...
						>
							...
							{#if cel.principal}
								<span
									class="text-[9px] px-1 py-0.2 bg-brand-gold/15 text-brand-gold border border-brand-gold/25 rounded-md uppercase font-bold tracking-wide"
									>Principal</span
								>
							{/if}
						</button>
		...
		{#if activeCelebration}
			<div class="flex-1 min-h-0 flex flex-col md:flex-row gap-6 w-full relative">
				...
				<nav
					class="hidden md:flex flex-col w-52 shrink-0 py-2 space-y-1 select-none sticky top-0 self-start"
				>
					<span class="text-[9px] uppercase font-bold tracking-widest text-text-muted/40 px-3 mb-2"
						>Sumário Litúrgico</span
					>
				...
				<div
					class="flex md:hidden overflow-x-auto shrink-0 gap-2 py-1.5 select-none border-b border-border-dark/40 w-full scrollbar-none"
				>
					{#each sections as sec}
						<button
							onclick={() => scrollToSection(sec.id)}
				...
				>
					...
					>
						...
						<h1
							class="text-base sm:text-lg font-bold tracking-tight leading-snug mt-3 text-text-main font-sans"
						>
							{activeCelebration.liturgia}
						</h1>
					</div>
					...
					{#each activeCelebration.leituras as leitura, idx (leitura.ordem)}
						...
						>
							...
							<div
								class="flex flex-col relative border-l-4 transition-colors duration-200
							...
							>
								<!-- Card Header -->
								<div
									class="p-5 pb-3 border-b border-border-dark/40 bg-bg-card/40 select-none flex items-center justify-between"
								>
									<div class="flex items-center gap-2">
										<div
											class="h-6 w-6 rounded-md flex items-center justify-center shrink-0 border border-brand-gold/20 bg-brand-gold-dim"
										>
								...
								<div class="p-6 space-y-4 flex-1">
									{#each leitura.opcoes as opcao}
										<div class="space-y-4">
											{#if opcao.titulo}
												<div
													class="rounded-xl p-3.5 select-none text-left border bg-bg-dark/40 border-border-dark/60"
												>
													<h3
														class="text-xs sm:text-sm font-bold leading-snug text-text-main font-sans"
													>
														{opcao.titulo}
													</h3>
												</div>
											...
											<div
												class="{readerFontSize} font-serif leading-relaxed text-justify select-all whitespace-pre-line pr-1 text-text-main"
											>
												{opcao.texto}
											</div>
										</div>
								...
								{#if activeCelebration.oracoes.extras && activeCelebration.oracoes.extras.length > 0}
									{#each activeCelebration.oracoes.extras as extra}
										<div class="space-y-2 border-t border-border-dark/40 pt-4">
											...
											<p
												class="{readerFontSize} font-sans leading-relaxed text-justify whitespace-pre-line text-text-main"
											>
												{extra.texto}
											</p>
										</div>
					...
					>
						...
						>
							...
							<div class="p-6 space-y-6">
								...
								<div class="space-y-2">
									<h3
										class="text-xs font-bold uppercase tracking-wider select-none text-brand-gold"
									>
										Antífona de Entrada
									</h3>
									<p class="{readerFontSize} font-sans leading-relaxed text-justify text-text-main">
